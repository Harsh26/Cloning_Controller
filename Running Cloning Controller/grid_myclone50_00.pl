...
startcontroller:-
   ...
   init_need(0),                    % Initially Need is 0, meaning that node will require service but it is not satisfied yet
   assert(satisfied_need(0)),       % Initially Need is not satisfied
   assert(pheromone_now('None')),   % Initially there is no pheromone
   assert(pheromone_time(1)),       % Initially pheromone time is 1
