...
q_manager_handle(_,(IP,NP),recv_agent(X, Token)):-
                ...
                q_manager_handle_thread(ID, (IP, NP), X, Token),
                !.
...
% update_bufferin_queue/1 updates the intranode_queue with the new queue list
:- dynamic update_bufferin_queue/1.
update_bufferin_queue(Inew):-

                        %writeln('Here in update function, Agent added to the queue':Inew),
                        retractall(bufferin(_)),
                        asserta(bufferin(Inew)),

                        !.
update_bufferin_queue(Inew):- write('update_bufferin_queue Failed'),!.
...
take_in_buffer([H|T]):-
        ...
        intranode_queue(I),
        agent_list_new(Aglist),
        (member(H, Aglist) -> enqueue(H, I, In), update_intranode_queue(In) ; nothing),
        take_in_buffer(T),
        !.
