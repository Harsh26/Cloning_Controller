...
:-dynamic enqueue_port/1.

:-dynamic dequeue_port/1.
...
q_manager(P):-
              ...
              assert(q_port(P)), % add fact to prolog db
              write('=====Q-Manager====='),
...
q_manager_handle(guid,(IP,P),recv_agent(X)):-
                                            writeln('request received for the arrival of agent':X),
                                             intranode_queue(Li), length(Li,Q_Len),queue_length(Len), %Len = 5, Li= [], length returns true if Q_Len mathces size of Li
                                             ack_q(ACK_Q),length(ACK_Q,ACK_Len), % ACK_Q = []
                                             L is ACK_Len + Q_Len,
                                             ((L<Len)->
                                             (
                                                       ack_q(Ack_QQ), % ACK_QQ = []
                                                       (
                                                         (member([X,_],Ack_QQ))-> % member returns true if [X, _] is member of list ACK_QQ
                                                                 (
                                                                 ...
                                                                 (
                                                                   ...
                                                                   ack_q(Ack_Q),
                                                                   q_port(QP),
                                                                   append([X],Ack_Q,Nw_Ack_Q) , % Append ACK_Q and X to Nw_ACK_Q
                                                                   agent_post(platform,(IP,P),[dq_manager_handle,dq_manager,(localhost,QP),ack(X)]), % predicate for platform, receiver IP = IP, rec port=P, then predicate list.
                                                                   writeln('Space is available in the queue. ACK sent.'),
                                                                   writeln('Ack Queue':Nw_Ack_Q),
                                                                   update_ack_queue(Nw_Ack_Q) % defined below, just retracting old values and asserting the new ones.
...
initiate_migration(GUID):-      % Used in dq_manager
                                clone_if_necessary(GUID), % based on paper
...
dq_manager_handle(Name,(IP,P),release(Q)):-
                        ...
                        agent_execute(Agent,(AIP,AP),Handler,move(localhost,P_tomove)),writeln(' SEMWAL!!!'); % move is predicate name like main that can be used for calling.. Main executes as a default handler, but if we need not execute default handler we can give some other name
