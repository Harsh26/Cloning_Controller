:- dynamic mobile_handler/3, static_handler/3, handler_test/3, handler1/3, val/2.

handler_test(guid,(_,_),main):-
    writeln('Hi').

create_send:-
    create_mobile_agent(agent, ('localhost',50001), handler1, [1,2]),
    retractall(val(_,_)),
    Value is 0,
    assert(val(guid, Value)), % Payload can have guid as a value
    add_payload(agent,[(val,2)]),
    val(_,Check),
    writeln('Value Asserted: ':Check),
    move_agent(agent, ('localhost',50002)).

handler1(guid, (_,50001),main):- % At 50001 guid is replaced by name of agent
    writeln('The current agent is: ':guid),
    writeln("Component of sender"),
    val(guid, Recieved_val),
    writeln('Received value: ':Recieved_val).

handler1(guid, (_,50002),main):- % At 50002 
    writeln('The current agent is: ':guid),
    writeln("Component of Reciever"),
    retractall(val(guid,_)),
    Send_val is 1,
    assert(val(guid, Send_val)),
    writeln('Sending value: ':Send_val),
    move_agent(agent,('localhost', 50001)).
