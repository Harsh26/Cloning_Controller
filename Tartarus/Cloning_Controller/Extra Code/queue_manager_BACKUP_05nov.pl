                                                       ...
                                                       (
                                                         (member([X,_],Ack_QQ))->
                                                                 ...
                                                                 (
                                                                   ...
                                                                   writeln('Ack Queue':Ack_Q),
                                                                   update_ack_queue(Ack_Q)
...
migrate_typhlet(GUID):-
                 ...
                 ack_q(ACK_Q),
                 writeln('Migrate ACK Q ':ACK_Q),
                (
                        member(GUID,ACK_Q)->
                                        (
                                                delete(GUID,ACK_Q,NwACK_Q),
...
migrate_typhlet(GUID):- write('Migrate Typhlet in Q-Manager Failed for':GUID),!.
...
dq_manager_handle(guid,(IP,P),nack(X,Y)):- writeln('dq_manager_handle NAK Failed'),!.
...
timer_release:-
               sleep(0.1),release_agent,timer_release,!.
